...
def amplifier_info(filename, omit_reference_pixels=True):
    ...
    amp_bounds : dict
        ...
        The first tuple gives the coordinates of the (minimum x, maximum x,
        and x step value), and the second tuple gives the (minimum y,
        maximum y, and y step value). These are set up such that a list of
        indexes for each amplifier can be generated by using
